// based on https://github.com/Tencent/ncnn/blob/5148224516b6864945fe77b4e57717cfa9f37b2d/tools/onnx/onnx2ncnn.cpp
...
static std::vector<int> get_node_attr_ai(const ONNX_NAMESPACE::NodeProto& node, const char* key)
{
    ...
    for (int i{ 0 }; i < node.attribute_size(); ++i)
    {
        ...
        if (attr.name() == key)
        {
            ...
            {
                v[j] = std::max(
                    std::min(attr.ints(j), (::google::protobuf::int64)INT_MAX), (::google::protobuf::int64)INT_MIN);
            }
...
static void set_node_attr_ai(ONNX_NAMESPACE::NodeProto& node, const char* key, const std::vector<int>& value)
{
    ONNX_NAMESPACE::AttributeProto* attr_group{ node.add_attribute() };
...
static std::string get_node_attr_s(
    const ONNX_NAMESPACE::NodeProto& node, const char* key, const std::string& def = std::string())
{
    for (int i{ 0 }; i < node.attribute_size(); ++i)
    {
        const ONNX_NAMESPACE::AttributeProto& attr = node.attribute(i);
...
static float get_node_attr_from_input_f(const ONNX_NAMESPACE::TensorProto& tp)
{
    float v{ 0.f };
...
static std::vector<int> get_node_attr_from_input_ai(const ONNX_NAMESPACE::TensorProto& tp)
{
    int size{ 0 };
    ...
    if (tp.data_type() == 7)
    {
        ...
        {
            int vi{ static_cast<int>(std::max(std::min(shape_data[j], (::google::protobuf::int64)INT_MAX), (::google::protobuf::int64)INT_MIN)) };
            v.push_back(vi);
        }
    }
    // int32
...
static std::vector<float> get_node_attr_from_input_af(const ONNX_NAMESPACE::TensorProto& tp)
{
    int size{ 0 };
...
static void fwrite_tensor_proto_data(const ONNX_NAMESPACE::TensorProto& tp, FILE* bp)
{
...
static void fuse_rewrite_gather(
    ONNX_NAMESPACE::GraphProto* mutable_graph, std::map<std::string, ONNX_NAMESPACE::TensorProto>& weights,
    std::map<std::string, int>& node_reference, std::set<std::string>& blob_names, int& reduced_node_count)
{
    ...
    {
        ONNX_NAMESPACE::NodeProto* gather{ mutable_graph->mutable_node(i) };
    ...
    std::map<std::string, int>& node_reference, std::set<std::string>& blob_names, int& reduced_node_count)
{
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "Transpose")
        {
            ...
            // permute weight
            {
                ONNX_NAMESPACE::TensorProto& B = weights[node->output(0)];

                const int h{ static_cast<int>(B.dims(0)) };
                const int w{ static_cast<int>(B.dims(1)) };
    ...
    std::map<std::string, int>& node_reference, std::set<std::string>& blob_names, int& reduced_node_count)
{
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "Reshape")
        {
            ...
            ONNX_NAMESPACE::NodeProto* node3{ mutable_graph->mutable_node(i + 2) };
            ...
            ONNX_NAMESPACE::AttributeProto* attr_group{ node3->add_attribute() };
            attr_group->set_name("group");
            attr_group->set_i(shape[1]);

            ONNX_NAMESPACE::AttributeProto* attr_reverse{ node3->add_attribute() };
    ...
    std::map<std::string, int>& node_reference, std::set<std::string>& blob_names, int& reduced_node_count)
{
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "ShuffleChannel")
        {
            ...
            ONNX_NAMESPACE::AttributeProto* attr_axis{ node3->add_attribute() };
    ...
    std::map<std::string, int>& node_reference, std::set<std::string>& blob_names, int& reduced_node_count)
{
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "Add")
        {
            ...
            const ONNX_NAMESPACE::TensorProto& add_three = weights[node->input(1)];
            ...
            ONNX_NAMESPACE::NodeProto* node2{ mutable_graph->mutable_node(i + 1) };
            ONNX_NAMESPACE::NodeProto* node3{ mutable_graph->mutable_node(i + 2) };
            ONNX_NAMESPACE::NodeProto* node4{ mutable_graph->mutable_node(i + 3) };
            ...
            {
                const ONNX_NAMESPACE::TensorProto& min_tp = weights[node2->input(1)];
                const ONNX_NAMESPACE::TensorProto& max_tp = weights[node2->input(2)];
            ...
            const ONNX_NAMESPACE::TensorProto& div_six = weights[node4->input(1)];
            ...
            ONNX_NAMESPACE::AttributeProto* attr_alpha{ node4->add_attribute() };
            attr_alpha->set_name("alpha");
            attr_alpha->set_f(1.f / 6.f);

            ONNX_NAMESPACE::AttributeProto* attr_beta{ node4->add_attribute() };
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "HardSigmoid")
        {
            ...
            ONNX_NAMESPACE::AttributeProto* attr_alpha{ node2->add_attribute() };
            attr_alpha->set_name("alpha");
            attr_alpha->set_f(alpha);

            ONNX_NAMESPACE::AttributeProto* attr_beta{ node2->add_attribute() };
            attr_beta->set_name("beta");
    ...
    std::map<std::string, int>& node_reference, std::set<std::string>& blob_names, int& reduced_node_count)
{
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "Add")
        {
            ...
            const ONNX_NAMESPACE::TensorProto& div_six = weights[node3->input(1)];
            if (div_six.dims_size() != 0 || get_tensor_proto_data_size(div_six) != 1)
                continue;
            ...
            ONNX_NAMESPACE::AttributeProto* attr_alpha{ node3->add_attribute() };
            attr_alpha->set_name("alpha");
            attr_alpha->set_f(1.f / 6.f);

            ONNX_NAMESPACE::AttributeProto* attr_beta{ node3->add_attribute() };
            attr_beta->set_name("beta");
            attr_beta->set_f(3.f / 6.f);
    ...
    std::map<std::string, int>& node_reference, std::set<std::string>& blob_names, int& reduced_node_count)
{
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "ReduceL2")
        {
            ...
            ONNX_NAMESPACE::NodeProto* node_shape{ 0 };
            if (has_shape_node)
            {
                if (i + 4 >= node_count)
                    continue;
    ...
    std::map<std::string, int>& node_reference, std::set<std::string>& blob_names, int& reduced_node_count)
{
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "Reshape")
        {
            ...
            ONNX_NAMESPACE::NodeProto* node3{ mutable_graph->mutable_node(i + 2) };
            ONNX_NAMESPACE::NodeProto* node4{ mutable_graph->mutable_node(i + 3) };
            ONNX_NAMESPACE::NodeProto* node5{ mutable_graph->mutable_node(i + 4) };
            ...
            ONNX_NAMESPACE::AttributeProto* attr_groups{ node5->add_attribute() };
            attr_groups->set_name("groups");
            attr_groups->set_i(groups);

            ONNX_NAMESPACE::AttributeProto* attr_channels{ node5->add_attribute() };
            attr_channels->set_name("channels");
            attr_channels->set_i(channels);

            ONNX_NAMESPACE::AttributeProto* attr_eps{ node5->add_attribute() };
            attr_eps->set_name("epsilon");
            attr_eps->set_f(eps);

            ONNX_NAMESPACE::AttributeProto* attr_affine{ node5->add_attribute() };
    ...
    std::map<std::string, int>& node_reference, std::set<std::string>& blob_names, int& reduced_node_count)
{
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "ReduceMean")
        {
            ...
            ONNX_NAMESPACE::NodeProto* node4{ mutable_graph->mutable_node(i + 3) };
            ONNX_NAMESPACE::NodeProto* node5{ mutable_graph->mutable_node(i + 4) };
            ONNX_NAMESPACE::NodeProto* node6{ mutable_graph->mutable_node(i + 5) };
            ONNX_NAMESPACE::NodeProto* node7{ mutable_graph->mutable_node(i + 6) };

            if (node2->op_type() != "Sub" || node3->op_type() != "Pow" || node4->op_type() != "ReduceMean" ||
                node5->op_type() != "Add" || node6->op_type() != "Sqrt" || node7->op_type() != "Div")
                continue;
            ...
            const ONNX_NAMESPACE::TensorProto& pow_two = weights[node3->input(1)];
            ...
            const ONNX_NAMESPACE::TensorProto& add_eps = weights[node5->input(1)];
            ...
            {
                ONNX_NAMESPACE::NodeProto* node8{ mutable_graph->mutable_node(i + 7) };
                ONNX_NAMESPACE::NodeProto* node9{ mutable_graph->mutable_node(i + 8) };
            ...
            if (affine == 0)
            {
                ...
                ONNX_NAMESPACE::AttributeProto* attr_eps{ node7->add_attribute() };
                attr_eps->set_name("epsilon");
                attr_eps->set_f(eps);

                ONNX_NAMESPACE::AttributeProto* attr_affine{ node7->add_attribute() };
                attr_affine->set_name("affine");
            ...
            else  // if (affine == 1)
            {
                ...
                ONNX_NAMESPACE::AttributeProto* attr_eps{ node9->add_attribute() };
                attr_eps->set_name("epsilon");
                attr_eps->set_f(eps);

                ONNX_NAMESPACE::AttributeProto* attr_affine{ node9->add_attribute() };
                attr_affine->set_name("affine");
                attr_affine->set_i(affine);
    ...
    std::map<std::string, int>& node_reference, std::set<std::string>& blob_names, int& reduced_node_count)
{
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "Shape")
        {
            ...
            if (node2->op_type() != "Gather" || node3->op_type() != "Constant" || node4->op_type() != "Unsqueeze" ||
                node5->op_type() != "Unsqueeze" || node6->op_type() != "Concat" || node7->op_type() != "Reshape")
                continue;
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "LSTM" || node->op_type() == "GRU" || node->op_type() == "RNN")
        {
            ...
            if (i + 1 < node_count)
            {
                ...
                ONNX_NAMESPACE::NodeProto* node4{ mutable_graph->mutable_node(i + 1) };
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "LSTM" || node->op_type() == "GRU" || node->op_type() == "RNN")
        {
            ...
            if (i + 1 < node_count)
            {
                ...
                ONNX_NAMESPACE::NodeProto* node3{ mutable_graph->mutable_node(i + 1) };
    ...
    std::map<std::string, int>& node_reference, std::set<std::string>& blob_names, int& reduced_node_count)
{
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "MatMul")
        {
            ...
            ONNX_NAMESPACE::NodeProto* node8{ mutable_graph->mutable_node(i + 7) };
            ONNX_NAMESPACE::NodeProto* node9{ mutable_graph->mutable_node(i + 8) };
            ONNX_NAMESPACE::NodeProto* node10{ mutable_graph->mutable_node(i + 9) };
            ONNX_NAMESPACE::NodeProto* node11{ mutable_graph->mutable_node(i + 10) };
            ONNX_NAMESPACE::NodeProto* node12{ mutable_graph->mutable_node(i + 11) };
            ONNX_NAMESPACE::NodeProto* node13{ mutable_graph->mutable_node(i + 12) };
            ONNX_NAMESPACE::NodeProto* node14{ mutable_graph->mutable_node(i + 13) };
            ONNX_NAMESPACE::NodeProto* node15{ mutable_graph->mutable_node(i + 14) };
            ONNX_NAMESPACE::NodeProto* node16{ mutable_graph->mutable_node(i + 15) };
            ONNX_NAMESPACE::NodeProto* node17{ mutable_graph->mutable_node(i + 16) };
            ONNX_NAMESPACE::NodeProto* node18{ mutable_graph->mutable_node(i + 17) };
            ONNX_NAMESPACE::NodeProto* node19{ mutable_graph->mutable_node(i + 18) };
            ONNX_NAMESPACE::NodeProto* node20{ mutable_graph->mutable_node(i + 19) };

            if (node2->op_type() != "Add" || node3->op_type() != "MatMul" || node4->op_type() != "Add" ||
                node5->op_type() != "MatMul" || node6->op_type() != "Add" || node7->op_type() != "Mul" ||
                node8->op_type() != "Reshape" || node9->op_type() != "Transpose" || node10->op_type() != "Reshape" ||
                node11->op_type() != "Reshape" || node12->op_type() != "Transpose" ||
                node13->op_type() != "Transpose" || node14->op_type() != "MatMul" || node15->op_type() != "Softmax" ||
                node16->op_type() != "MatMul" || node17->op_type() != "Transpose" || node18->op_type() != "Reshape" ||
            ...
            int embed_dim{ static_cast<int>(q_B.size()) };
            ...
            ONNX_NAMESPACE::AttributeProto* attr_embed_dim{ node20->add_attribute() };
            attr_embed_dim->set_name("embed_dim");
            attr_embed_dim->set_i(embed_dim);

            ONNX_NAMESPACE::AttributeProto* attr_num_heads{ node20->add_attribute() };
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        if (node->op_type() == "MatMul")
        {
            ...
            if (node2->op_type() != "Add" || node3->op_type() != "Split" || node4->op_type() != "Mul" ||
                node5->op_type() != "Reshape" || node6->op_type() != "Transpose" || node7->op_type() != "Reshape" ||
                node8->op_type() != "Reshape" || node9->op_type() != "Transpose" || node10->op_type() != "Transpose" ||
                node11->op_type() != "MatMul" || node12->op_type() != "Softmax" || node13->op_type() != "MatMul" ||
                node14->op_type() != "Transpose" || node15->op_type() != "Reshape" || node16->op_type() != "MatMul" ||
                node17->op_type() != "Add")
                continue;
            ...
            int embed_dim{ static_cast<int>(o_B.size()) };
            ...
            ONNX_NAMESPACE::AttributeProto* attr_embed_dim{ node17->add_attribute() };
            attr_embed_dim->set_name("embed_dim");
            attr_embed_dim->set_i(embed_dim);

            ONNX_NAMESPACE::AttributeProto* attr_num_heads{ node17->add_attribute() };
            attr_num_heads->set_name("num_heads");
            attr_num_heads->set_i(num_heads);
    ...
    std::map<std::string, int>& node_reference, std::set<std::string>& blob_names, int& reduced_node_count)
{
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
            ...
            node->op_type() == "Pow")
        {
            ...
            const ONNX_NAMESPACE::TensorProto& scalar_b = weights[node->input(0)];
            ...
            ONNX_NAMESPACE::AttributeProto* attr_with_scalar{ node->add_attribute() };
            attr_with_scalar->set_name("with_scalar");
            attr_with_scalar->set_i(1);

            ONNX_NAMESPACE::AttributeProto* attr_b{ node->add_attribute() };
            attr_b->set_name("b");
            attr_b->set_f(b);
        }
    }
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
            ...
            node->op_type() == "Pow")
        {
            ...
            const ONNX_NAMESPACE::TensorProto& scalar_b = weights[node->input(1)];
            if (scalar_b.dims_size() != 0 || get_tensor_proto_data_size(scalar_b) != 1)
                continue;
...
std::optional<std::tuple<char*, unsigned char*>> onnx2ncnn(ONNX_NAMESPACE::ModelProto& model)
{
    auto pp{ std::tmpfile() };
    auto bp{ std::tmpfile() };
    ...
    // topological sort
    {
        ...
        for (int i{ 0 }; i < node_count;)
        {
            ...
            for (; q < node_count; ++q)
            {
                ONNX_NAMESPACE::NodeProto* nodeq{ mutable_graph->mutable_node(q) };
                bool found{ false };
            ...
            ONNX_NAMESPACE::NodeProto* nodeq{ mutable_graph->mutable_node(q) };
            ONNX_NAMESPACE::NodeProto tmp{ *node };
            *node = *nodeq;
            *nodeq = tmp;
        }
    }
    ...
    for (std::map<std::string, ONNX_NAMESPACE::TensorProto>::iterator it{ weights.begin() }; it != weights.end(); ++it)
    {
    ...
    fprintf(
        ...
        reduced_node_count + input_node_count + split_layer_count,
    ...
    for (std::map<std::string, ONNX_NAMESPACE::TensorProto>::iterator weight_it{ weights.begin() }; weight_it != weights.end();
        ++weight_it)
    {
        ...
        const ONNX_NAMESPACE::TensorProto& M = weights[input_name];
    ...
    for (int i{ 0 }; i < node_count; ++i)
    {
        ...
        */
        ...
        else if (op == "Reciprocal")
        {
            ...
            op == "ReduceMax" || op == "ReduceMin" || op == "ReduceMean" || op == "ReduceProd" || op == "ReduceSum" ||
            op == "ReduceSumSquare" || op == "ReduceL1" || op == "ReduceL2" || op == "ReduceLogSum" ||
            op == "ReduceLogSumExp")
        {
        ...
        else if (op == "Clip")
        {
            ...
            {
                min = weights.find(node.input(1)) != weights.end() ? get_node_attr_from_input_f(weights[node.input(1)])
                    : -FLT_MAX;
                max = weights.find(node.input(2)) != weights.end() ? get_node_attr_from_input_f(weights[node.input(2)])
                    : FLT_MAX;
            }
        ...
        else if (op == "Conv")
        {
            ...
            int num_filter{ static_cast<int>(W.dims(0)) };
        ...
        else if (op == "ConvTranspose")
        {
            ...
            std::vector<int> pads = get_node_attr_ai(node, "pads");
            int group{ get_node_attr_i(node, "group", 1) };
            int num_filter{ static_cast<int>(W.dims(1)) * group };
        ...
        {
            const ONNX_NAMESPACE::TensorProto& words = weights[node.input(2)];
            const ONNX_NAMESPACE::TensorProto& positions = weights[node.input(3)];
            const ONNX_NAMESPACE::TensorProto& W = weights[node.input(5)];
            const ONNX_NAMESPACE::TensorProto& B = weights[node.input(6)];
        ...
        else if (op == "adaptive_avg_pool2d" || op == "adaptive_max_pool2d")
        {
            ...
            const ONNX_NAMESPACE::TensorProto& out_shape_tp = weights[node.input(1)];
        ...
        {
            const ONNX_NAMESPACE::TensorProto& W = weights[node.input(1)];
            const ONNX_NAMESPACE::TensorProto& R = weights[node.input(2)];
            const ONNX_NAMESPACE::TensorProto& B = weights[node.input(3)];
        ...
        else if (op == "MatMul")
        {
            if (weights.find(node.input(1)) != weights.end() && weights[node.input(1)].dims_size() == 2)
            {
                ...
                int num_output{ static_cast<int>(B.dims(B.dims_size()) - 1) };
        ...
        else if (op == "MultiHeadAttention")
        {
            ...
            if (node.input_size() == 5)
            {
                const ONNX_NAMESPACE::TensorProto& qkvw = weights[node.input(1)];
                const ONNX_NAMESPACE::TensorProto& qkvb = weights[node.input(2)];
                const ONNX_NAMESPACE::TensorProto& ow = weights[node.input(3)];
                const ONNX_NAMESPACE::TensorProto& ob = weights[node.input(4)];
            ...
            {
                const ONNX_NAMESPACE::TensorProto& qw = weights[node.input(3)];
                const ONNX_NAMESPACE::TensorProto& qb = weights[node.input(4)];
                const ONNX_NAMESPACE::TensorProto& kw = weights[node.input(5)];
                const ONNX_NAMESPACE::TensorProto& kb = weights[node.input(6)];
                const ONNX_NAMESPACE::TensorProto& vw = weights[node.input(7)];
                const ONNX_NAMESPACE::TensorProto& vb = weights[node.input(8)];
                const ONNX_NAMESPACE::TensorProto& ow = weights[node.input(9)];
                const ONNX_NAMESPACE::TensorProto& ob = weights[node.input(10)];
        ...
        {
            const ONNX_NAMESPACE::TensorProto& W = weights[node.input(2)];
            const ONNX_NAMESPACE::TensorProto& B = weights[node.input(3)];
            const ONNX_NAMESPACE::TensorProto& B2 = weights[node.input(4)];
        ...
        void* result { [&]()
{
    ...
    auto param{ static_cast<char*>(aligned_malloc(param_size, sizeof(void*))) };
    std::rewind(pp);
    ...
    auto model_bin{ static_cast<unsigned char*>(aligned_malloc(model_size, sizeof(void*))) };
    std::rewind(bp);
